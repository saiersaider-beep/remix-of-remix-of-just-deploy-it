...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.artist_followers TO authenticated;
...
CREATE POLICY "Users read audio" ON storage.objects FOR SELECT USING (bucket_id = 'audio');
...
CREATE POLICY "Users read covers" ON storage.objects FOR SELECT USING (bucket_id = 'covers');
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.track_reposts TO authenticated;
GRANT SELECT ON public.track_reposts TO anon;
GRANT ALL ON public.track_reposts TO service_role;
