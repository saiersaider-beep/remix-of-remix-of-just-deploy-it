...
DO $e$ BEGIN CREATE POLICY "Users read own listening history" ON public.listening_history FOR SELECT USING (auth.uid() = user_id); EXCEPTION WHEN OTHERS THEN NULL; END $e$;
DO $f$ BEGIN CREATE POLICY "Users insert own listening history" ON public.listening_history FOR INSERT WITH CHECK (auth.uid() = user_id); EXCEPTION WHEN OTHERS THEN NULL; END $f$;
DO $g$ BEGIN CREATE POLICY "Users delete own listening history" ON public.listening_history FOR DELETE USING (auth.uid() = user_id); EXCEPTION WHEN OTHERS THEN NULL; END $g$;
...
DO $h$ BEGIN CREATE POLICY "Users read own track access" ON public.track_access FOR SELECT USING (auth.uid() = user_id OR public.has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN OTHERS THEN NULL; END $h$;