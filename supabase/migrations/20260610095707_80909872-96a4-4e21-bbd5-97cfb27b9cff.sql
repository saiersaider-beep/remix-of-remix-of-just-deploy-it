...
DO $a2$ BEGIN
  CREATE TABLE IF NOT EXISTS public.messages (
    ...
    content text, audio_url text, transcript text,
...
CREATE TABLE IF NOT EXISTS public.artist_verification_requests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, artist_id uuid,
  stage_name text, legal_name text, documents jsonb,
...
CREATE TABLE IF NOT EXISTS public.payment_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  provider text NOT NULL, flw_tx_id text, flw_tx_ref text, cinetpay_tx_id text,
  event_type text, status text, amount integer, currency text,
  payload jsonb, processed boolean NOT NULL DEFAULT false,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.wallet_topup_requests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, amount_xof integer NOT NULL,
  method text NOT NULL DEFAULT 'bank',
  status text NOT NULL DEFAULT 'pending',
  reference text, receipt_url text, notes text,
  reviewed_by uuid, reviewed_at timestamptz,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.withdrawal_requests (
  ...
  destination jsonb, status text NOT NULL DEFAULT 'pending', notes text,
  reviewed_by uuid, reviewed_at timestamptz,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.copyright_claims (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  reporter_id uuid, track_id uuid, reason text, evidence_url text,
...
CREATE TABLE IF NOT EXISTS public.bank_receipts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, amount_xof integer, receipt_url text,
  status text NOT NULL DEFAULT 'pending',
  reviewed_by uuid, reviewed_at timestamptz,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.banned_ips (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ip text NOT NULL UNIQUE, reason text, banned_by uuid,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.branding (
  ...
  logo_url text, favicon_url text, primary_color text, app_name text,
  updated_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.reports (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  reporter_id uuid, target_type text NOT NULL, target_id uuid,
...
CREATE TABLE IF NOT EXISTS public.artist_daily_stats (
  artist_id UUID NOT NULL, day DATE NOT NULL,
  plays INTEGER NOT NULL DEFAULT 0, likes INTEGER NOT NULL DEFAULT 0,
  followers_gained INTEGER NOT NULL DEFAULT 0, followers_lost INTEGER NOT NULL DEFAULT 0,
